...
public:
    AStar(int width, int height, const Point &start_, const Point &end_) :
        gridWidth(width), gridHeight(height), start(start_), end(end_)
    {
        ...
        while (!openSet.empty()) {
            ...
            for (int x = -1; x <= 1; x++) {
                for (int y = -1; y <= 1; y++) {
                    ...
                    Node *successor = new Node(newCoordinates, currentNode); // Node(coord, parent)
                    successor->gCost = newGCost;
                    successor->hCost = heuristic(successor->coordinates, end);

                    // If allNodes does not contain 'successor->coordinates' it will create a new element
                    // wich is a Node*. And this pointer will be a nullptr.
                    if (Node *existingNode = allNodes[successor->coordinates]; existingNode != nullptr) {
                        // existingNode was not created by default constructor
                        // this Node* was alrady in allNodes
                        if (existingNode->gCost <= successor->gCost) {
                            delete successor;
                            continue; // stop executing here and go the next interation
                        }
